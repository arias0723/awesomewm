...
local list_update = function(widget, buttons, label, data, objects)
	...
	for i, object in ipairs(objects) do
		...
		local tag_widget_margin = wibox.widget({
			tag_widget,
			margins = dpi(2),
			widget = wibox.container.margin,
		})
		...
		tag_widget:connect_signal("mouse::enter", function()
			...
			if object == awful.screen.focused().selected_tag then
				tag_widget.bg = beautiful.groups_bg .. 55
			else
				tag_widget.bg = beautiful.groups_bg
			end
		...
		tag_widget:connect_signal("button::press", function()
			if object == awful.screen.focused().selected_tag then
				tag_widget.bg = beautiful.press_event .. 55
			else
				tag_widget.bg = beautiful.press_event
			end
		end)

		tag_widget:connect_signal("button::release", function()
			if object == awful.screen.focused().selected_tag then
				tag_widget.bg = beautiful.release_event .. 55
			else
				tag_widget.bg = beautiful.release_event
			end
		end)
		...
		widget:add(tag_widget_margin)
		-- widget:set_spacing(dpi(3))
	end
end
